...
/******************************************************************************/
/*!
\brief	Set the X and Y elements of this vector
...
*/
/******************************************************************************/
void Vector3::SetXY(float a, float b)
{
	x = a;
	y = b;
}
...
float Vector3::GetDistance(Vector3& check)
{
	if (!Same(check))
...
float Vector3::GetDistanceSquared(Vector3& check)
{
	if (!Same(check))
		return (*this - check).LengthSquared();
	else
		return 0.f;
}

Vector3 Vector3::GetDirection(Vector3& check)
{
	if (!Same(check))
		return (check - *this).Normalized();
	else
		return Vector3(1, 0, 0);
}

bool Vector3::Same(Vector3& check)
{
	return abs(x - check.x) <= Math::EPSILON &&
		abs(y - check.y) <= Math::EPSILON &&
		abs(z - check.z) <= Math::EPSILON;
}
...
*/
/******************************************************************************/
float Vector3::getAngleFromDir(float dirX, float dirY) throw( DivideByZero )
{
	return atan2(dirY, dirX) * 180 / PI;
}

Vector3& Vector3::multiplyWithVector(Vector3& rhs)
{
	x *= rhs.x;
	y *= rhs.y;
	z *= rhs.z;
...
Vector3& Vector3::divideWithVector(Vector3& rhs)
{
	x /= rhs.x;
	y /= rhs.y;
	z /= rhs.z;
